---
title: "Reproducible Research: Peer Assessment 1"
output: 
  html_document:
    keep_md: true
---


## Loading and preprocessing the data
```{r load, echo=TRUE}
mytable <- read.table("activity.csv", header = TRUE, sep = ",", stringsAsFactors = TRUE)

```

Here is a summary table
```{r mysummary, results='asis'}
library(knitr)
kable(summary(mytable))
```


## What is mean total number of steps taken per day?
I create a new dataframe variable called nonna without NA's. Here is a summary table

```{r nona, echo=TRUE}
nonna<-na.omit(mytable)
kable(summary(nonna))
```

Using dplyr package I calculate the total number of steps per day.
Then a histogram is plotted

```{r groupingdate, echo=TRUE}
library(dplyr)

mygroup<-group_by(nonna,nonna$date)
steps_day<-summarise(mygroup,total=sum(steps), .groups= 'drop')
hist(steps_day$total,
     col = "peachpuff",
     border = "black",
     xlab = "Total number of steps per day",
     main = "Histogram: Total Number of Steps per Day")

```

The mean and median are very close.

```{r computeeda, echo=TRUE}

mymean <- mean(steps_day$total)
mymedian <- median(steps_day$total)

```

The mean of the total number of steps is `r mymean` and the median is `r mymedian`


## What is the average daily activity pattern?



```{r groupinginterval, echo=TRUE}

myinterval<-group_by(nonna,nonna$interval)
steps_daily<-summarise(myinterval,totalmean=mean(steps), .groups= 'drop')
plot(steps_daily$`nonna$interval`, steps_daily$totalmean, type = "l")

```


## Imputing missing values



## Are there differences in activity patterns between weekdays and weekends?
